...
当时的意图是先把“圆桌感”建立起来，而不是只生成几个普通聊天角色。遇到的挑战是 AI 容易生成相似专家或漏掉主持人，因此通过“只生成 1 位 host”“expertCount 位 expert”“不同职业背景和立场”等硬性约束引导 AI 修正。
...
当时的意图是让 AI 像真实圆桌一样根据上下文接话，而不是按专家列表机械轮流。主要挑战是模型容易解释“为什么选择某人发言”或暴露内部调度，所以我反复加入“不输出内部流程”“content 只放用户可见发言”等约束来修正输出。
...
当时的意图是让页面展示自然语言总结，同时后端仍能拿到结构化数据。遇到的挑战是模型可能把总结写成纯列表或编造 transcript 中不存在的结论，因此通过“主持人口吻”“不编造 transcript 外结论”“summary.content 必须自然语言”来引导 AI 修正。
...
当时的意图是先把作业要求转成可执行规格，避免项目变成普通聊天页。挑战是需求截图里信息很多且分散，所以先引导 AI 提炼用户画像、流程、API、数据结构和验收标准。
...
当时的意图是让后续代码围绕领域对象展开，而不是让页面状态随意增长。挑战是 `DiscussionState`、`DiscussionSession`、`Message` 等命名容易混乱，所以通过要求定义核心类型和模块职责来让 AI 收敛设计。
...
当时的意图是先让测试描述业务行为，再补实现代码。挑战是早期引擎函数还不存在，所以测试先暴露缺口，再引导 AI 逐步实现 `createDiscussionSession`、`addMessageToSession`、`createFinalSummary` 等最小逻辑。
...
当时的意图是从用户视角检查“创建、讨论、追问、总结、导出”是否完整。挑战是项目最初只是半交互演示，E2E 检查暴露出前端未调用 API、共识分歧未展示等问题，随后按 bug list 引导 AI 一项项修正。
